                ...
                public void run()
                {
                    ...
                    try
                    {
                        ...
                        if (offlinePayments.size() > 0)
                        {
                            ...
                            {
                                player.sendMessage(ChatColor.WHITE + "Transaction " + index + ": " + paymentRecord.transactionDate());

                                player.spigot().sendMessage(MessageGenerator.generateTipReceiverMessage(paymentRecord));
                                player.spigot().sendMessage(MessageGenerator.generateBlockExplorerLink(configEngine, paymentRecord.blockHash()));
